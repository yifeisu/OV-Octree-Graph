...
class OVSegPredictor(DefaultPredictor):
    ...
    def forward_regions(self, original_image, regions, device):
        """
        Args:
            original_image (np.ndarray): an image of shape (H, W, C) (in BGR order).
        ...
        with torch.no_grad():  # https://github.com/sphinx-doc/sphinx/issues/4258
            ...
            height, width = original_image.shape[:2]
            # image = self.aug.get_transform(original_image).apply_image(original_image)
            # print(self.aug.get_transform(original_image))

            image = torch.as_tensor(original_image.astype("float32").transpose(2, 0, 1)).to(device)

            inputs = {"image": image, "height": height, "width": width, "class_names": None}
            predictions = self.model.clip_region_inference(inputs, regions)
...
class VisualizationDemo(object):
    ...
    def run_on_regions(self, image, regions, device):
        region_features = self.predictor.forward_regions(image, regions, device)
