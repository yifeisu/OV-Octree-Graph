...
class MaskFormerClipAdapter(ClipAdapter):
    ...
    def get_vl_featrues(
        ...
        if isinstance(regions, list):
            assert NotImplementedError
            image_features = torch.cat([self.get_image_features(image_i) for image_i in regions], dim=0)
    ...
    def _preprocess_image(
        ...
        # cv2.imwrite(f"ovseg_rgb.jpg", image.cpu().permute(1, 2, 0).numpy().astype(np.uint8))
        # cv2.imwrite(f"ovseg_region.jpg", regions[0].squeeze().cpu().permute(1, 2, 0).numpy().astype(np.uint8))
        # sys.exit(0)
