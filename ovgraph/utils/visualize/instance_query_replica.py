...
def visualize_scene_graph(scene_pcd, scene_graph, show_center=False):
    ...
    for idx, node_id in enumerate(node_ids):
        ...
        vis_colors[pt_indices] = colors[idx]
        if show_center:
...
def visualize_instances(scene_pcd, scene_graph):
    ...
    def show_next_instance_callback(visualizer, action, mod, pressed_key):
        ...
        print(f"{scene_graph.nodes[instance_id]['top1_cls']}")
...
def main():
    ...
    parser.add_argument("-v", "--video", type=str, default="room0")

    # replica
    parser.add_argument("--prediction_file", default="proposed_fusion_cropformer-none-0.5-ovseg_tap_iou-0.25_recall-0.75_feature-0.75_interval-50.pkl")
    ...
    scene_pcd_path = video_path / f"{args.video}_mesh.ply"
